...
class trajetsFerroviaires:
    ...
    def get_donneesFerroviaires(self) -> List[dict]:
        self.conn.row_factory = sqlite3.Row
        self.cursor = self.conn.cursor()
    ...
    def get_CO2_data(self) -> List[dict]:
        self.conn.row_factory = sqlite3.Row
        self.cursor = self.conn.cursor()
        self.cursor.execute("SELECT * FROM carbon_dioxide")
        rows = self.cursor.fetchall()
...
donneesFerroviaires = trajetsFerroviaires("../bdd/trajetsFerroviairesBDD.db")
...
def obetnir_trajets_inter_Pays():
    return donneesFerroviaires.get_donneesFerroviaires()
...
@app.get("/donnesCO2", response_model=List[dict])
def obetnir_donnees_CO2():
    return donneesFerroviaires.get_CO2_data()

@app.get("/itineraire")
def calculer_itineraire(depart: str, arrivee: str):
    itineraire = donneesFerroviaires.calculer_itineraire(depart, arrivee)
