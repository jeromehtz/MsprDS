...
la persistance effective en base et les différents filtres.

Les tests utilisent la base de données de test (SQLite/PostgreSQL)
et ne dépendent pas de Business Central.
"""
...
def test_create_then_list_trajet(client, auth_headers, db_session):
    ...
    # Vérification de la persistance directement en base
    count = db_session.query(Trajet).count()
    assert count == 1

    # Vérification que le trajet est également exposé par l'API
    listing = client.get(
        "/trajets/",
        headers=auth_headers,
    )
...
def _seed(client, headers, **overrides):
    """
    Ajoute un trajet de test en réutilisant le payload de base.
    """
    payload = {
        **TRAJET,
        **overrides,
...
def test_filter_by_service_type_and_region(client, auth_headers):
    ...
    res = client.get(
        "/trajets/",
        params={"service_type": "TER/Intercités"},
        headers=auth_headers,
    )
...
def test_filter_by_search_on_station_name(client, auth_headers):
    ...
    _seed(
        client,
        auth_headers,
        destination_station_name="Dijon Ville",
    )

    res = client.get(
        "/trajets/",
        params={"search": "marseille"},
        headers=auth_headers,
    )
    ...
    assert len(data) == 1
    assert "Marseille" in data[0]["destination_station_name"]
...
def test_filter_limit(client, auth_headers):
    for i in range(5):
        _seed(
            client,
            auth_headers,
            destination_station_name=f"Gare {i}",
        )
